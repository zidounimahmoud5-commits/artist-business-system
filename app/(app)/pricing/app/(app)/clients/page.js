    ...
    <div>
      ...
      {detail && (
        <Modal title={detail.name} onClose={() => setDetail(null)}>
          ...
          {sales.filter((s) => s.client_id === detail.id).length === 0 ? <div style={{ fontSize: 13.5, color: "#8A8371" }}>{c.noPurchases}</div> : sales.filter((s) => s.client_id === detail.id).map((s) => (
          ...
          <div style={{ marginTop: 14 }}><Btn variant="ghost" onClick={() => { setFormModal({ mode: "edit", client: detail }); setDetail(null); }}>{t.common.edit}</Btn></div>
        </Modal>
      )}
    </div>
  );
...
function ClientForm({ t, mode, initial, onClose, onSave }) {
  ...
  return (
    <Modal title={mode === "new" ? c.add : c.name} onClose={onClose}>
      ...
      <Field label={c.type}><Select value={form.type} onChange={(e) => set("type", e.target.value)}>{CLIENT_TYPE_KEYS.map((k) => <option key={k} value={k}>{t.clientType[k]}</option>)}</Select></Field>
