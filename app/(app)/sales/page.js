  ...
  return (
    <div>
      <SectionTitle right={sales.length > 0 && <Btn variant="ghost" onClick={() => exportCsv("sales.csv", sales.map((x) => ({
        artwork: artworkMap[x.artwork_id]?.title || x.artwork_id, buyer: clientMap[x.client_id]?.name || "", date: x.date, price: x.price, paymentMethod: x.payment_method, netRevenue: x.net_revenue, profit: x.profit,
      })))}>{s.exportCsv}</Btn>}>{s.title}</SectionTitle>
      {sales.length === 0 ? <EmptyState text={s.empty} /> : (
        <Card style={{ padding: 0, overflowX: "auto" }}>
          <table>
            <thead><tr><th>{s.artwork}</th><th>{s.buyer}</th><th>{s.date}</th><th>{s.price}</th><th>{t.paymentMethod.label}</th><th>{s.net}</th><th>{s.profit}</th></tr></thead>
            <tbody>
              {sales.map((x) => (
                <tr key={x.id}>
                  ...
                  <td>{t.paymentMethod[x.payment_method] || "—"}</td>
