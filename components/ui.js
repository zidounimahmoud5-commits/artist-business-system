...
const inputStyle = { width: "100%", padding: "9px 12px", borderRadius: 8, border: `1px solid ${BORDER}`, background: SURFACE_SOLID, fontSize: 14.5, color: INK, fontFamily: "inherit", boxSizing: "border-box" };
...
export function Btn({ children, onClick, variant = "primary", style = {}, type = "button", disabled }) {
  ...
  const variants = {
    ...
    ghost: { background: SURFACE_SOLID, color: INK, border: `1px solid ${BORDER}` },
    danger: { background: SURFACE_SOLID, color: DANGER, border: `1px solid ${DANGER_BORDER}` },
  };
...
export function Card({ children, style = {} }) {
  return (
    <div style={{
      background: SURFACE, backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)",
      border: `1px solid ${BORDER}`, borderRadius: 14, padding: 20, boxShadow: CARD_SHADOW, ...style,
    }}>
      {children}
    </div>
  );
...
export function Modal({ title, onClose, children, wide }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(30,26,22,0.5)", zIndex: 50, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "4vh 16px", overflowY: "auto" }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: SURFACE_ALT, backdropFilter: "blur(14px)", WebkitBackdropFilter: "blur(14px)", borderRadius: 16, padding: 24, width: "100%", maxWidth: wide ? 720 : 480, marginTop: 20, boxShadow: "0 20px 60px rgba(30,20,10,0.28)" }}>
...
export function StatTile({ label, value, sub }) {
  return (
    <div style={{ padding: "16px 18px", background: SURFACE, backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)", border: `1px solid ${BORDER}`, borderRadius: 12, minWidth: 150, flex: "1 1 150px", boxShadow: CARD_SHADOW }}>
...
export function SectionTitle({ children, right }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, marginTop: 4, flexWrap: "wrap", gap: 10 }}>
      <h2 style={{ fontFamily: "var(--font-display)", fontSize: 24, fontWeight: 600, margin: 0, position: "relative", paddingBottom: 10, color: INK, textShadow: "0 1px 12px rgba(255,253,249,0.8)" }}>
        {children}
